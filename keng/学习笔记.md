...
```
+号是Java提供的一个语法糖。语法糖指的是计算机语言中添加的某种语法，这种语法对语言的功能没有影响，但是更方便程序员的使用。经过反编译，s3的内容为
s3 = (new StringBuilder()).append("he").append("llo").toString()。也就是说，Java中的+对字符串的拼接，其实现原理是使用StringBuilder.append。
