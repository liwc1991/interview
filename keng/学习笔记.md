...
### 限流发送
使用了google.common.util.concurrent.RateLimiter类，里面有一个setRate方法是设置发送速率key，key是配置库中配置的每秒的令牌数目。key的值越大，表明发送速率越大，例key=0.5，即一秒处理0.5个10000的用户。在交互中心的特护推送中，6点前设置为0.3，之后考虑到网络空闲，可以调整到1。
